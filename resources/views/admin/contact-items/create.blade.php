        ...
        <form action="{{ isset($contactItem) ? route('admin.contact-items.update', $contactItem) : route('admin.contact-items.store') }}" method="POST">
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <small class="text-muted">Gunakan format <code>bi-nama-icon</code>. Lihat <a href="https://icons.getbootstrap.com/" target="_blank">bootstrap</a>.</small>
                    <div class="mt-2 d-flex flex-wrap gap-2">
                        @foreach(['bi-geo-alt-fill','bi-telephone-fill','bi-envelope-fill','bi-whatsapp','bi-globe','bi-clock-fill','bi-building-fill','bi-person-fill'] as $ic)
                        <button type="button" class="btn btn-sm btn-outline-secondary icon-pick-btn" data-icon="{{ $ic }}" title="{{ $ic }}">
                            <i class="bi {{ $ic }}"></i>
                        </button>
                        @endforeach
                    </div>
